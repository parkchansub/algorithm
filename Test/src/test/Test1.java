...
public class Test1 {
	...
	public static void main(String[] args) throws IOException {
		...
		a = new int[N][N]; visit = new int[N][N]; bridge = new int[N][N];
		for (int i = 0; i < N; i++) {
	...
	public static void bfs() {
		while (list.size() != 0) {
			...
			for (int i = 0; i < 4; i++) {
				int sx = point.x + dx[i], sy = point.y + dy[i];
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				for (int k = 0; k < 4; k++) {
					...
					if (sx >= 0 && sx < N && sy >= 0 && sy < N && a[i][j] != a[sx][sy]) { min = Math.min(min, bridge[sx][sy] + bridge[i][j]); }
				}
			}
		}
		return min;
